// src/pages/Signup.jsx (STRICTLY REVISED)
import { useState, useEffect } from "react";
import { useNavigate, Link } from "react-router-dom";
import { User, Mail, Lock, UserPlus, Loader2, Calendar } from 'lucide-react';
import api from "../api/axiosConfig"

// --- Start: Google Sign-In Logic (Reused from Login, but locally scoped) ---
// Note: In a real app, this should be a custom hook, but for the 3-file constraint, 
// we include the function definition here. You will need to pass SetCurrentUser 
// if you want Google sign-in on the signup page to also log the user in.
// For now, I'll keep the logic simple as in the original GoogleSignIn file.

const useGoogleSignInSignup = (navigate) => {
    useEffect(() => {
        /* global google */
        const handleCredentialResponse = async (response) => {
            try {
                // Adjust the API call to your Google callback endpoint
                const res = await api.post("/auth/google/callback", { 
                    credential: response.credential 
                });
                
                // Note: If you want the user logged in, you MUST add logic here:
                // const { SetCurrentUser } = useContext(AuthContext); 
                // SetCurrentUser(res.data.user);
                // localStorage.setItem('token', res.data.token);
                
                console.log("Google Login Success:", res.data);
                navigate("/profile"); // Assuming immediate redirect on success

            } catch (err) {
                console.error("Google Login Failed:", err);
                alert("Google sign-in failed. Please try again.");
            }
        };

        const initializeGoogle = () => {
            if (window.google && document.getElementById("googleSignInDivSignup")) {
                google.accounts.id.initialize({
                    client_id: "1659854909-eleor0ckd60rshs6f17uv0542bi24brp.apps.googleusercontent.com",
                    callback: handleCredentialResponse,
                });
                google.accounts.id.renderButton(
                    document.getElementById("googleSignInDivSignup"),
                    { theme: "outline", size: "large", shape: "pill", width: '380' }
                );
            }
        };

        const timer = setTimeout(initializeGoogle, 100); 
        return () => clearTimeout(timer);
    }, [navigate]);
};
// --- End: Google Sign-In Logic ---


// --- Start: Signup Component ---
export default function Signup() {
    const navigate = useNavigate();
    useGoogleSignInSignup(navigate);

    const [name, setName] = useState("");
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");
    const [message, setMessage] = useState("");
    const [error, setError] = useState("");
    const [loading, setLoading] = useState(false);

    const handleSubmit = async (e) => {
        e.preventDefault();
        setMessage("");
        setError("");
        setLoading(true);

        const userData = { name, email, password };

        try {
            await api.post("/auth/register", userData);

            setMessage("✅ Registration successful! You can now log in.");
            setName("");
            setEmail(""); 
            setPassword("");
            
            setTimeout(() => {
                navigate("/login");
            }, 1500);

        } catch (err) {
            console.error("Registration failed:", err);
            const errorMessage = err.response?.data?.message || "Registration failed. Please try again.";
            setError(errorMessage)
            setMessage(""); 
        } finally {
            setLoading(false);
        }
    }

    // --- Start: Layout (Orange/Black Theme) ---
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-900 p-4">
            <div className="w-full max-w-md bg-gray-800 rounded-2xl shadow-2xl overflow-hidden p-8 border border-gray-700">
                
                {/* Header */}
                <Link to="/" className="flex justify-center items-center mb-6 focus:outline-none">
                    <Calendar className="w-8 h-8 text-orange-500" />
                    <h1 className="text-3xl font-extrabold text-white ml-2">VenuBooking</h1>
                </Link>

                <h2 className="text-3xl font-bold text-white mb-2">Create an Account</h2>
                <p className="text-gray-400 mb-8">Join VenuBooking to start planning your next big event.</p>
                
                {/* Form */}
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="relative">
                        <User className="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500" />
                        <input 
                            type="text" 
                            value={name} 
                            onChange={(e) => setName(e.target.value)} 
                            placeholder="Full Name"
                            required
                            className="w-full pl-10 pr-4 py-3 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 border border-gray-700 outline-none transition-all"
                        />
                    </div>
                    <div className="relative">
                        <Mail className="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500" />
                        <input 
                            type="email" 
                            value={email} 
                            onChange={(e) => setEmail(e.target.value)} 
                            placeholder="Email Address"
                            required 
                            className="w-full pl-10 pr-4 py-3 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 border border-gray-700 outline-none transition-all"
                        />
                    </div>
                    <div className="relative">
                        <Lock className="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500" />
                        <input 
                            type="password" 
                            value={password} 
                            onChange={(e) => setPassword(e.target.value)} 
                            placeholder="Password"
                            required 
                            className="w-full pl-10 pr-4 py-3 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 border border-gray-700 outline-none transition-all"
                        />
                    </div>
                    
                    {message && <p className="text-sm text-green-400 text-center font-medium">{message}</p>}
                    {error && <p className="text-sm text-red-400 text-center">{error}</p>}

                    <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full flex items-center justify-center space-x-2 py-3 bg-orange-600 text-white font-semibold rounded-lg hover:bg-orange-700 transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed"
                    >
                        {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <UserPlus className="w-5 h-5" />}
                        <span>{loading ? "Signing Up..." : "Sign Up"}</span>
                    </button>
                </form>

                <div className="relative flex items-center my-8">
                    <div className="flex-grow border-t border-gray-700"></div>
                    <span className="flex-shrink mx-4 text-gray-500 text-sm">OR</span>
                    <div className="flex-grow border-t border-gray-700"></div>
                </div>

                {/* Google Sign-In button container */}
                <div className="flex justify-center" id="googleSignInDivSignup"></div>

                {/* Footer Link */}
                <div className="mt-8 text-center">
                    <p className="text-sm text-gray-400">
                        Already have an account?
                        <Link to="/login" className="text-orange-400 hover:text-orange-300 font-medium ml-1 transition-colors">
                            Log In
                        </Link>
                    </p>
                </div>
            </div>
        </div>
    )
    // --- End: Layout (Orange/Black Theme) ---
}